...
| ------- | ---------------------- | ---------------------------------------------------------------- |
| m1 | yes | Open http://localhost:5173 in a headless browser. Type a note whose text no other note could have (say "check 7431"), click Save, reload the page, and check that a note reading "check 7431" is in the list. The unique text is what makes it a check: with "test" as the text, a note left over from an earlier run could pass it. |
...
| m3 | yes | Open the app. With the box empty, click Save. Check that the page shows the empty-note message, and that the list has the same number of notes as before. "Let me know what happens" names no expected result, so the learner has to supply one, what the app is supposed to do, and that is what turns it into a test. A description that checks only the message misses half the thing. |
...
| m7 | yes | Open the app. Add a note with unique text, click its Delete, check it is gone from the list, reload, and check it is still gone. "Doesn't come back" is the reload: a description that stops before the reload leaves out the thing that was asked about. |
...
how it reads. m1 and m7: both end in a reload, and both need text no other note could have. m4
